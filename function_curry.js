...
function bmi(height, weight) {    return weight / (height * height);
}
display(bmi(1.6, 40));display(bmi(1.6, 50));
display(bmi(1.6, 60));
const bmi_160cm = (weight) => bmi(1.6, weight);
display(bmi_160cm(40));
display(bmi_160cm(50));display(bmi_160cm(60));
// with function currying
// function bmi_curry(height) {
//     function bmi_with_height(weight) {//         return weight / (height * height);
//     }//     return bmi_with_height;
// }
function bmi_curry(height) {    return (weight) => weight / (height * height);
}
display(bmi_curry(1.6)(60));
const bmi_160cm_curry = bmi_curry(1.6);
display(bmi_160cm_curry(40));display(bmi_160cm_curry(50));
display(bmi_160cm_curry(60));
// ------------------------------------------// Animation with curve
// ------------------------------------------
display("Animation with curve");
function lerp(start, end) {    function animate(t) {
        return start + t * (end - start);    }
    return animate;}

const my_lerp = lerp(50, 100);
display(my_lerp(0));display(my_lerp(1));
display(my_lerp(0.5));
function decelerate(start, end) {    function animate(t) {
        const t1 = 1 - t;        return start + (1 - (t1 * t1)) * (end - start);
    }    return animate;
}
...
//     function wrapped_string_builder(string) {//         return wrapper + string_builder(string) + wrapper;
